...
!!! originally in helm_table_strage.dek
  ...
  double precision, save :: tlo,thi,tstp,tstpi,dlo,dhi,dstp,dstpi
  ...
  ! The Helmholtz interpolation table and its grid metadata are initialized
  ! once by init_eos, before any OpenMP parallel region.  EOS evaluations only
  ! read this shared state; the table is intentionally not thread-local.
  ...
  real(8), save :: temp_eos_min, temp_eos_max
contains

  subroutine init_eos(fn_eos_in)
    character(*),intent(in)  :: fn_eos_in

    ! Initialization mutates the shared table and must finish before EOS
    ! evaluation begins in an OpenMP parallel region.
    call read_helm_table(fn_eos_in)
  ...
  recursive subroutine eos_all(rho_in, temp_in, ye_in, ytot_in, mexc_in, &
  ...
  recursive subroutine eos_get_hmin(mexc, hmin)
  ...
  recursive subroutine eos_get_eps_min(rho, ye, mexc, eps_min)
  ...
  recursive subroutine eos_get_range(temp_min_out,temp_max_out,rhoye_min_out,rhoye_max_out)
  ...
  subroutine read_helm_table(fn_eos)
    ! include 'implno.dek'
    ! include 'helm_table_storage.dek'

    ! This routine reads the Helmholtz EOS file once, before scalar EOS
    ! routines are called from an OpenMP parallel region.

    character(*), intent(in) :: fn_eos
  ...
  recursive subroutine eos_get_misc(rho_in, temp_in, ye_in, ytot_in, mexc_in, &
       eta_out)
    real(8),intent(in) :: rho_in, temp_in, ye_in, ytot_in, mexc_in
  ...
  recursive subroutine eos_get_temp_from_pres(pres, rho, ye, ytot, temp, temp_lo, err_return, return_code_T)
...
!!!
  ...
  recursive subroutine master_function_temp_from_pres(logtemp, f, pres_target, rho, ye, ytot, mexc, pres_min)
    use, intrinsic :: ieee_arithmetic, only : ieee_is_finite
    ! log10(T (K))
  ...
  recursive subroutine eos_get_temp_from_eps(eps, rho, ye, ytot, mexc, temp, temp_lo, err_return, return_code_T)
    use, intrinsic :: ieee_arithmetic, only : ieee_is_finite
    implicit none
    ! epsilon (erg/g)
  ...
  recursive subroutine master_function_temp_from_eps(logtemp, f, eps_target, rho, ye, ytot, mexc, eps_min)
  ...
  recursive function eos_get_eps_at_temp_min(rho,ye,ytot,mexc) result(eps)
    ! rho [g cm^-3]
    real(8) :: rho
